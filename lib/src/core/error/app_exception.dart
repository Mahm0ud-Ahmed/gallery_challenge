      ...
      switch (dioError.type) {
        case DioExceptionType.connectionTimeout:
          errorModel = ErrorModel(
            ...
            errorMessage: '$typeName${S.current.connect_timeout_message}',
          );
          break;
        case DioExceptionType.sendTimeout:
          errorModel = ErrorModel(
            statusCode: dioError.response?.statusCode,
            errorMessage: '$typeName${S.current.send_timeout_message}',
          );
          break;
        case DioExceptionType.receiveTimeout:
          errorModel = ErrorModel(
            statusCode: dioError.response?.statusCode,
            errorMessage: '$typeName${S.current.receive_timeout_message}',
          );
          break;
        case DioExceptionType.badCertificate:
          errorModel = ErrorModel(
            statusCode: dioError.response?.statusCode,
            errorMessage: '$typeName${S.current.bad_certificate_message}',
          );
          break;
        case DioExceptionType.connectionError:
          errorModel = ErrorModel(
            statusCode: dioError.response?.statusCode,
            errorMessage: '$typeName${S.current.connection_error_message}',
          );
          break;
        case DioExceptionType.cancel:
          errorModel = ErrorModel(
            statusCode: dioError.response?.statusCode,
            errorMessage: '$typeName${S.current.cancel_error_message}',
          );
          break;
        ...
        case DioExceptionType.unknown:
          ...
          errorModel = ErrorModel(errorMessage: message);
          break;
      }
    }
    else if (error is TypeError && error.toString().contains('is not a subtype of type')) {
      AppLogger.logError(error);
      errorModel = ErrorModel(errorMessage: S.current.error_occurred_while_processing_the_data_received_from_the_server_message);
    } else {
      errorModel = ErrorModel(errorMessage: S.current.unknown_error_message);
    }
    return errorModel;
  }
